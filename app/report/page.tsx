...
import { RevenueGrowthChart, InvestmentHistoryChart, MarketPresenceMap } from './CompanyIntelligenceCharts';
        ...
        <div className={gridCols ? `grid gap-6 ${gridCols}` : ''}>
          ...
          {analysisText && (
             <div className={`${gridCols && items && items.length > 0 ? 'col-span-1' : ''} ${!gridCols && items && items.length === 0 ? 'mt-0' : gridCols ? 'md:mt-0' : 'mt-4' }`}>
              ...
              <p className={`text-gray-700 whitespace-pre-line leading-relaxed text-sm`}>{analysisText}</p>
            </div>
          )}

          {children && (items || analysisText ? <div className={gridCols ? 'md:col-span-full' : ''}>{children}</div> : children)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-6xl mx-auto py-20 px-8">
        ...
        <section className="mb-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {architecture?.overall_score !== undefined && (
              <div className="p-8 bg-gradient-to-br from-white via-blue-50 to-white rounded-2xl shadow-lg">
                <h3 className="text-lg font-medium text-blue-600 mb-3">Architecture Score</h3>
                <p className="text-4xl font-bold text-blue-700">{architecture.overall_score}/10</p>
              </div>
            )}
            {security?.overall_score !== undefined && (
              <div className="p-8 bg-gradient-to-br from-blue-50 via-sky-100 to-blue-50 rounded-2xl shadow-lg">
                <h3 className="text-lg font-medium text-sky-600 mb-3">Security Score</h3>
                <p className="text-4xl font-bold text-sky-700">{security.overall_score}/10</p>
              </div>
            )}
            {report?.adamassSynthesisReport?.overall_assessment?.confidence_score !== undefined && (
              <div className="p-8 bg-gradient-to-br from-white via-cyan-50 to-white rounded-2xl shadow-lg">
                <h3 className="text-lg font-medium text-cyan-600 mb-3">Adamass Confidence</h3>
          ...
          {companyIntelligence?.error ? (
            ...
            <div className="space-y-0">
              ...
              <DetailedInfoCard title="Company Overview">
                <div className="grid md:grid-cols-3 gap-8 items-start">
                  <div className="md:col-span-2">
                    <p className="text-gray-700 leading-relaxed mb-6 text-base">{companyIntelligence.company_overview?.overview || 'No overview available.'}</p>
                    {companyIntelligence.company_overview?.company_mission && (
                      <div className={`p-5 bg-blue-50 border-l-4 border-blue-400 rounded-r-md`}>
                        <h4 className={`text-lg font-semibold text-blue-700 mb-2`}>Mission Statement</h4>
                        <p className={`text-blue-700 whitespace-pre-line leading-relaxed text-sm`}>{companyIntelligence.company_overview.company_mission}</p>
                      </div>
                    )}
                  </div>
                  <div className="p-6 bg-gray-50 border border-gray-200 rounded-xl shadow-sm space-y-3 text-sm">
                    <h4 className="text-lg font-semibold text-gray-700 mb-3">Key Information</h4>
                    {companyIntelligence.company_overview?.official_company_name && <p><strong className="text-gray-600">Official Name:</strong> {companyIntelligence.company_overview.official_company_name}</p>}
                    {companyIntelligence.company_overview?.industry && <p><strong className="text-gray-600">Industry:</strong> <span className="bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded-full text-xs font-medium">{companyIntelligence.company_overview.industry}</span></p>}
                    {companyIntelligence.company_overview?.headquarters && <p><strong className="text-gray-600">Headquarters:</strong> {companyIntelligence.company_overview.headquarters}</p>}
                    {companyIntelligence.company_overview?.founding_date && <p><strong className="text-gray-600">Founded:</strong> {companyIntelligence.company_overview.founding_date}</p>}
                    {companyIntelligence.company_overview?.number_of_employees && <p><strong className="text-gray-600">Employees:</strong> {companyIntelligence.company_overview.number_of_employees}</p>}
                    {companyIntelligence.company_overview?.website && <p><strong className="text-gray-600">Website:</strong> <a href={companyIntelligence.company_overview.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{companyIntelligence.company_overview.website}</a></p>}
                  </div>
                </div>
              </DetailedInfoCard>
              ...
              <DetailedInfoCard title="Financials & Metrics">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4 mb-6 text-gray-700 text-sm">
                  <div><strong className="text-gray-600">Revenue:</strong> {companyIntelligence.financials_metrics?.revenue ?? 'N/A'}</div>
                  <div><strong className="text-gray-600">Profit:</strong> {companyIntelligence.financials_metrics?.profit ?? 'N/A'}</div>
                  <div><strong className="text-gray-600">EBITDA:</strong> {companyIntelligence.financials_metrics?.ebitda ?? 'N/A'}</div>
                  <div><strong className="text-gray-600">IT Spend:</strong> {companyIntelligence.financials_metrics?.it_spend ?? 'N/A'}</div>
                  <div><strong className="text-gray-600">Web Visits:</strong> {companyIntelligence.financials_metrics?.web_visits ?? 'N/A'}</div>
                  <div><strong className="text-gray-600">Active Tech:</strong> {companyIntelligence.financials_metrics?.active_website_tech_count ?? 'N/A'}</div>
                </div>
                {companyIntelligence.financials_metrics?.growth_scores && Object.keys(companyIntelligence.financials_metrics.growth_scores).length > 0 && (
                  <div className="mb-6">
                    <h4 className="text-md font-semibold text-gray-700 mb-2">Growth Scores (YoY):</h4>
                ...
                {companyIntelligence.financials_metrics?.financial_commentary && 
                  <div className={`p-5 bg-green-50 border-l-4 border-green-400 rounded-r-md mb-4`}>
                    <h4 className={`text-lg font-semibold text-green-700 mb-2`}>Financial Commentary</h4>
                    <p className={`text-green-700 whitespace-pre-line leading-relaxed text-sm`}>{companyIntelligence.financials_metrics.financial_commentary}</p>
                  </div>
                }
                {companyIntelligence.financials_metrics?.financial_metrics_analysis && 
                  <div className={`p-5 bg-lime-50 border-l-4 border-lime-400 rounded-r-md`}>
                    <h4 className={`text-lg font-semibold text-lime-700 mb-2`}>Financial Metrics Deep Dive</h4>
                    <p className={`text-lime-700 whitespace-pre-line leading-relaxed text-sm`}>{companyIntelligence.financials_metrics.financial_metrics_analysis}</p>
                  </div>
                }
              </DetailedInfoCard>
              ...
              {/* Graphs Section - Remains the same, but ensure styling aligns with new card system margins */}
              {companyIntelligence.graph_data && (
                <div className="space-y-12 mt-10">
                  <DetailedInfoCard title="Revenue Growth">
                    <RevenueGrowthChart data={companyIntelligence.graph_data.revenue_growth} />
                  </DetailedInfoCard>
                  <DetailedInfoCard title="Investment History">
                    <InvestmentHistoryChart data={companyIntelligence.graph_data.investment_history} />
                  </DetailedInfoCard>
                </div>
              )}
            </div>
        ...
        {/* Adamass Intelligence Report Section - RESTORE */}
          ...
          <SectionCard title="Adamass Intelligence Report">
            <div className="space-y-10">
              ...
              {report.adamassSynthesisReport.executive_summary && (
                <div>
                  <h3 className="text-2xl font-semibold text-gray-800 mb-4">Executive Summary</h3>
                  <p className="text-gray-700 leading-relaxed whitespace-pre-line">{report.adamassSynthesisReport.executive_summary}</p>
                </div>
              )}
                ...
                <div className="p-6 bg-slate-50 rounded-lg">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-blue-600 mb-4">{report.adamassSynthesisReport.overall_assessment.verdict}</p>
                    ...
                    <div>
                      <p className="text-sm text-gray-500 mb-1">Key Rationale</p>
                      <p className="text-gray-700 leading-relaxed bg-slate-100 p-4 rounded-md">{report.adamassSynthesisReport.overall_assessment.key_rationale}</p>
                    </div>
                  </div>
                </div>
              )}
                    ...
                    {report.adamassSynthesisReport.key_risks_and_mitigation.map((item: any, index: number) => (
                      <div key={index} className="p-4 rounded-lg bg-white border border-gray-200">
                        <div className="flex justify-between items-start mb-1">
                          <h4 className="text-md font-semibold text-red-700">Risk: {item.risk}</h4>
                          <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${item.severity === 'High' ? 'bg-red-100 text-red-700' : item.severity === 'Medium' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>
                            {item.severity} Severity
                          </span>
                        </div>
                        <p className="text-sm text-gray-600 mt-1 pl-1"><strong>Mitigation:</strong> {item.mitigation}</p>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-6">
                    ...
                    {report.adamassSynthesisReport.potential_synergies.cost_synergies && report.adamassSynthesisReport.potential_synergies.cost_synergies.length > 0 && (
                      <div className="p-5 bg-blue-50 rounded-lg">
                        <h4 className="text-lg font-semibold text-blue-700 mb-3">Cost Synergies</h4>
                        ...
                        <div className="space-y-3">
                          {report.adamassSynthesisReport.potential_synergies.cost_synergies.map((synergy: any, index: number) => (
                            <div key={index} className="p-3 rounded-md bg-white">
                              ...
                              <p className="text-xs text-gray-600 mt-0.5"><strong>Est. Savings:</strong> {synergy.estimated_annual_savings}</p>
                    ...
                    {report.adamassSynthesisReport.potential_synergies.revenue_synergies && report.adamassSynthesisReport.potential_synergies.revenue_synergies.length > 0 && (
                      <div className="p-5 bg-green-50 rounded-lg">
                        <h4 className="text-lg font-semibold text-green-700 mb-3">Revenue Synergies</h4>
                        ...
                        <div className="space-y-3">
                          {report.adamassSynthesisReport.potential_synergies.revenue_synergies.map((synergy: any, index: number) => (
                            <div key={index} className="p-3 rounded-md bg-white">
                              ...
                              <p className="text-xs text-gray-600 mt-0.5"><strong>Est. Revenue Inc.:</strong> {synergy.estimated_annual_revenue_increase}</p>
                              <p className="text-xs text-gray-600"><strong>Timeline:</strong> {synergy.realization_timeline}</p>
                            </div>
                          ))}
                        </div>
              ...
              {/* Strategic Recommendations - Component already handles its own light card styling */}
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {report.adamassSynthesisReport.strategic_recommendations.map((rec: any, index: number) => (
                      <StrategicRecommendationItem key={rec.id || index} recommendation={rec} index={index} />
                    ))}
                  </div>
                </div>
              )}
              ...
              {report.adamassSynthesisReport.closing_statement && (
                <div className="mt-8">
                  <h3 className="text-2xl font-semibold text-gray-800 mb-4">Closing Statement</h3>
                  <p className="text-gray-700 leading-relaxed whitespace-pre-line">{report.adamassSynthesisReport.closing_statement}</p>
                </div>
              )}
            </div>
          </SectionCard>
        )}
        {/* End of Adamass Intelligence Report Section */}
      </div>
    </main>
  );
};
